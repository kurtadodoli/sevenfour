...
try {
    ...
    if (content.includes(searchPattern)) {
        ...
        console.log('❌ Could not find the exact pattern to replace. Let me try a more specific search...');
        
        // Try alternative pattern
        const altPattern = `                                </InfoSection>
                              </ExpandedContent>`;
        
        if (content.includes(altPattern)) {
            const altReplacement = `                                </InfoSection>
            ...
            const updatedContent = content.replace(altPattern, altReplacement);
            fs.writeFileSync(filePath, updatedContent, 'utf8');
            console.log('✅ Successfully added buttons using alternative pattern!');
        } else {
            console.log('❌ Could not find suitable pattern. Manual edit required.');
        }
    }
