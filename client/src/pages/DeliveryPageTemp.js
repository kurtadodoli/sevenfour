      ...
      try {
        ...
        if (ordersResponse.data.success) {
          ...
          const processedOrders = ordersData.map(order => {
            ...
            // Log for debugging
            console.log(`Processing order ${order.id}: Set as ${orderType}`);
              ...
              <OrdersList>
                ...
                {(() => {
                  const filteredOrders = orders
                    .filter(order => {
                      // Simpler filtering logic with strict type checking
                      if (orderFilter === 'all') return true;
                      if (orderFilter === 'regular') return order.order_type === 'regular';
                      if (orderFilter === 'custom') return order.order_type === 'custom';
                      return true;
                    })
                    .filter(order => {
